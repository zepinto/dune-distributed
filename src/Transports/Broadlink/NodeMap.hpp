...
#ifndef DUNE_BROADLINK_NODE_MAP_HPP_INCLUDED_
#define DUNE_BROADLINK_NODE_MAP_HPP_INCLUDED_
...
namespace Transports
{
  namespace Broadlink
  {
    ...
    public:
      ...
      void
      add(const std::string& name, const unsigned& address)
      {
      ...
      bool
      lookupAddress(const std::string& name, unsigned& address) const
      {
        typename std::map<std::string, unsigned>::const_iterator itr = m_node_names.find(name);
      ...
      bool
      lookupName(const unsigned& address, std::string& name) const
      {
        typename std::map<unsigned, std::string>::const_iterator itr = m_node_addresses.find(address);
    ...
    private:
      ...
      std::map<std::string, unsigned> m_node_names;
      //! Map node addresses to names.
      std::map<unsigned, std::string> m_node_addresses;
    };
  }
}
